...
아래는 직접 구현한 lock 과 conditional 을 사용해 식사하는 철학자 문제를 구현한 것이다. 큰 로직은 다익스트라 방법이랑 비슷하다. 필요한 포크가 없으면 점유를 푼 뒤 대기하고 이웃은 포크를 다 쓴 후 대기를 풀어준다. 점유를 푸는 로직이 빠져도 무조건 포크 두개를 동시에 들어야 해서 데드락은 걸리지 않지만 효율성이 떨어진다.
